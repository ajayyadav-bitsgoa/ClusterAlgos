...
def expectation_maximization(t, label, nbclusters=2, nbiter=3, normalize=False,\
        epsilon=0.001, monotony=False, datasetinit=True):

    global purity_scores, nmi_scores
    purity_scores = list()
    nmi_scores = list()
    ...
    def pnorm(x, m, s):
        ...
        try:
            p = (2.0*math.pi)**(-len(x)/2.0)*(1.0/math.sqrt(np.linalg.det(s)))\
    ...
    for iteration in range(nbiter):
        ...
        while (abs(log_estimate - old_log_estimate) > epsilon\
                and (not monotony or log_estimate < old_log_estimate) and estimation_round <= estRoundLimit):
            restart = False
        ...
        # Pick/save the best clustering as the final result
            ...
            if estimation_round%4 ==0:
                print( "\nestimation", estimation_round)
                # i = 0
                # for a in params:
                #     print("cluster ", i," : \n" , "proba: ", a['proba'], " ,mean:  ", a['mu'])
                #     i = i +1
                # print("\n")
                getDistribution(result['clusters'], label, nbclusters, t.shape[0])
                print("(EM) old and new log estimate: ",old_log_estimate, log_estimate)
...
def getDistribution(arr, label, no_of_clusters, no_of_docs):
    global purity_scores, nmi_scores
    documentClusterDistribution = np.zeros(shape=(no_of_clusters, no_of_clusters))
    clusterClass = np.zeros(shape=(no_of_docs,))
    for i in range(0, len(arr)):
        for ele in arr[i]:
            documentClusterDistribution[i][label[ele]] += 1
            clusterClass[ele] = i

    print(documentClusterDistribution)
    purity = 0
    for i in range(0, len(documentClusterDistribution)):
        purity += max(documentClusterDistribution[i])
    purity /=  no_of_docs
